...
def compute_survey_limits(survey):
    ...
    # "Speed" metric (as in original code)
...
def plot_surveys(yaml_path, output="SETI_limits_comparison.pdf", publish=False, twocolumn=False, noshow=False):
    ...
    for survey in surveys:
        ...
        marker = survey.get("marker", "o")
        color = survey.get("color", "k")
        msize = survey.get("markersize", 18)
        label = survey.get("name", "survey")
    ...
    # -------------------------------------------------------
    plt.plot([17, 17], [-13, 6], '--', lw=4, color='black', alpha=0.6, label="Kardashev Type I")
    plt.plot([13, 13], [-13, 6], '-.', lw=4, color='black', alpha=0.6, label="Arecibo Planetary Radar")

    plt.xlabel(r"EIRP $[\log_{10}(\mathrm{W})]$", fontsize=20)
    plt.ylabel(
        r"Transmitter Rate "
    ...
    plt.legend(loc=1, prop={"size": 16 if not twocolumn else 10}, labelspacing=1, frameon=True)
