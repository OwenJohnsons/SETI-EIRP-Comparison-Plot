"""
Code Purpose: Main script to plot SETI EIRP limits from survey definitions in a YAML file.
...
def get_args():
    ...
    parser.add_argument(
        "-pub", "--publish", action="store_true",
        help="Format the plot for publication using scienceplots."
    )

    parser.add_argument(
        "--twocolumn", action="store_true",
...
def main():
    ...
    if args.input == "surveys.yaml":
        pkg_dir = pathlib.Path(__file__).resolve().parent
        yaml_path = pkg_dir / "surveys.yaml"
    else:
        yaml_path = pathlib.Path(args.input)
